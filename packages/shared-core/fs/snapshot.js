'use strict';
// fs/snapshot.js — 快照 / 回滚（manifest 含文件哈希；回滚先备份再覆盖再验证）
//
...
const { assertWithinRealpath } = require('../ids');
...
function restoreSnapshot(fsPort, snapshot, dir, opts = {}) {
  ...
  try {
    ...
    //    C-1 修复（阶段 1）：realpath 整路径比真根——junction/symlink 越界拒，
    //    并对解析后的路径做后续 I/O（无 TOCTOU））
    fsPort.mkdirSync(dir, { recursive: true });
    for (const f of snapshot.files || []) {
      if (f.type === 'link') continue;
      const lexical = path.join(dir, f.rel);
      const within = assertWithinRealpath(fsPort, dir, lexical, `快照文件 ${f.rel}`);
      if (!within.ok) return { ok: false, error: within.error };
      const dest = within.resolvedPath;
      fsPort.mkdirSync(path.dirname(dest), { recursive: true });
      try {
...
function cleanupResidue(fsPort, dir, opts = {}) {
  if (opts.root) {
    // C-1 修复（阶段 1）：realpath 越界校验——清理目标不得经符号链接逃出根域
    const within = assertWithinRealpath(fsPort, opts.root, dir, '清理目标');
    if (!within.ok) return { ok: false, error: within.error };
