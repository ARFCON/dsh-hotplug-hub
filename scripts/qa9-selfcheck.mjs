 ...
 *      DSH 版本 = 0.1.1-rc.1（绝不显示 24.18.0）、文案「本地 DSH（CLI/全局安装）」、
 *      最新版本 0.9.7 < 本程序 0.9.8 → 「已最新」（不误报可更新）
 *   B. 官方 Desktop 场景：dshDesktop 命中 → 「官方 Harness 已安装」，版本取官方描述
 *   C. 完全无 DSH：→ 「未检测到 DSH」+ warn
...
try {
  ...
  const runScene = async (mock, label) => {
    ...
    await page.click('#nav button[data-view="check"]')
  ...
  check('DSH 版本文案 = 本地 DSH（CLI/全局安装）', a.dsh && a.dsh.text.includes('本地 DSH'), a.dsh && a.dsh.text)
  ...
  check('官方场景：文案 = 官方 Harness 已安装', b.dsh && b.dsh.text.includes('官方 Harness 已安装'), b.dsh && b.dsh.text)
  ...
  check('无 DSH：文案 = 未检测到 DSH', c.dsh && c.dsh.text.includes('未检测到 DSH'), c.dsh && c.dsh.text)
