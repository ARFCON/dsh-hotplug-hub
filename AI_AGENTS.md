...
## 14. 每次修改必须更新 开发文档/更新历史.md

- 每次完成修改/修复/新功能后，必须把本次改动整合成「更新历史」写到 `开发文档/更新历史.md`（README 只保留一行入口链接）。
- 格式：在最新版本小节下追加 `### <版本> 修复/新增` 列表。
- 不更新历史不允许提交推送。
...
- 每次更新完成后：自动 `git push`，并同步更新 `开发文档/更新历史.md`。
...
### 任务结束后（AI 自动执行）
- [ ] 已自动执行任务审查（不再询问）
- [ ] 已自动检查：同步 / 更新历史 / 全局记忆 / check-before-upload
