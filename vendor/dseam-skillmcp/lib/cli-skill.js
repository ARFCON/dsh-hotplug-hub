#!/usr/bin/env node
/**
 * dseam-skillmcp skill —— dseam-skillmcp 插件的技能热管理命令行。
 *
 ...
 *
 *   dseam-skillmcp skill list                     列出技能（标注全局 / 工作区）
 *   dseam-skillmcp skill enable <name>            重新启用已停用的技能
 *   dseam-skillmcp skill disable <name>           热停用技能（改名 *.disabled）
 *   dseam-skillmcp skill delete <name> [--yes]    永久删除技能
 *   dseam-skillmcp skill add <path>               添加技能（.md 文件、目录束或 .zip 压缩包）
 *   dseam-skillmcp skill scope <name>             迁移单个技能：--global | --workspace <path>
 *   dseam-skillmcp skill migrate <name...>        批量迁移：--from --to [--copy] [--all]
 *   dseam-skillmcp skill update [--yes]           检查并更新插件（默认 --profile web）
...
function usage() {
    console.log([
        "用法:",
        "  dseam-skillmcp skill list [--cwd <path>]                         列出技能（标注全局 / 工作区）",
        "  dseam-skillmcp skill enable <name> [--cwd <path>]                启用已停用的技能",
        "  dseam-skillmcp skill disable <name> [--cwd <path>]               停用技能（改名 *.disabled，热生效）",
        "  dseam-skillmcp skill delete <name> [--yes] [--cwd <path>]        删除技能（目录型删整个目录）",
        "  dseam-skillmcp skill add <path> [--cwd <path>] [--project | --workspace <path>]",
        "                                                        添加技能：.md 文件、目录束或 .zip 压缩包（自动识别结构）",
        "  dseam-skillmcp skill scope <name> [--global | --workspace <path>] [--copy]",
        "                                                        迁移单个技能到全局或指定工作区（默认移动，--copy 复制）",
        "  dseam-skillmcp skill migrate <name...|--all> --from <global|路径> --to <global|路径> [--copy] [--yes]",
        "                                                        批量迁移：把源工作区的技能复制/移动到目标工作区",
        "  dseam-skillmcp skill update [--yes] [--profile <name>]          检查最新版本，有更新则自动安装（默认 web 配置）",
        "",
...
if (directPath !== undefined && (directPath === modulePath || resolve(directPath) === resolve(modulePath))) {
    runSkillCli(process.argv.slice(2)).then((code) => {
        ...
        console.error("dseam-skillmcp skill: " + (error instanceof Error ? error.message : String(error)));
        process.exitCode = 1;
    });
}
