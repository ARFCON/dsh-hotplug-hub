# dsh-skill/mcp-panel — dseam-skillmcp
...
1. Install the package (its bundle layer auto-mounts it — no config editing)
   ...
   ```bash
   dsh plugin --profile web add https://github.com/Fishquito7/dseam-skillmcp/releases/download/v2.0.1/dseam-skillmcp-2.0.1.tgz
   ```
   ...
   >
   > ```bash
   > dsh plugin --profile web add github:Fishquito7/dseam-skillmcp
   > ```
...
The package ships the unified `dseam-skillmcp` command. Skill management:

```bash
dseam-skillmcp skill --help
```

MCP management:

```bash
dseam-skillmcp skill list                                  # list skills (with scope: global / workspace)
dseam-skillmcp skill add <path>                            # add to global (.md file, bundle dir, or .zip archive)
dseam-skillmcp skill add <path> --workspace D:\projA       # add directly into a workspace
dseam-skillmcp skill scope <name> --global                  # migrate one skill to global
dseam-skillmcp skill scope <name> --workspace D:\projA      # migrate one skill into a workspace (--copy to copy)
dseam-skillmcp skill migrate <name...|--all> --from <global|path> --to <global|path> [--copy] [--yes]
dseam-skillmcp skill update [--profile <name>]  # check for updates and install (default profile: web)
                                                 # batch migrate (copy or move)
dseam-skillmcp skill disable <name>       # disable
dseam-skillmcp skill enable <name>        # enable
dseam-skillmcp skill delete <name>        # delete (asks for confirmation)
```
...
```bash
dseam-skillmcp mcp list [--profile <name>]
dseam-skillmcp mcp add --name <serverName> --stdio --command <cmd> [--args <arg> ...] [--env KEY=VALUE ...] [--cwd <path>] [--profile <name>]
dseam-skillmcp mcp add --name <serverName> --http --url <url> [--header KEY=VALUE ...] [--profile <name>]
dseam-skillmcp mcp enable|disable <serverName> [--profile <name>]
dseam-skillmcp mcp remove <serverName> [--yes] [--profile <name>]
dseam-skillmcp mcp test <serverName> [--profile <name>]
dseam-skillmcp mcp update [--yes] [--profile <name>]
dseam-skillmcp update [--yes] [--profile <name>]      # update the whole package
```

MCP configuration is written to the managed block in the target profile's `cordis.patch.yml`
(markers `# >>> dseam-skillmcp:mcp:begin` / `# <<< ...end`). Do not edit inside the block.

The plugin doesn't parse skills itself — it's just a management surface over the skill files: every action in the page (or via `dseam-skillmcp skill`) ends up as a change to the skill files on disk (`SKILL.md`), and DSH's own file watcher notices immediately. That's why enable/disable, add, delete and migration are all hot — no gateway restart.
...
```bash
dsh plugin --profile web remove dseam-skillmcp
```
