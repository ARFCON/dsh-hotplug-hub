...
DSH 插件，在 Web 设置页同时提供「技能」与「MCP」两个管理面板，并随包提供统一终端命令 `dseam-skillmcp`（`skill` / `mcp` 两个子命令族）。
...
```bash
dseam-skillmcp skill list                                  # 列出技能（含工作区：全局 / 工作区）
dseam-skillmcp skill add <path>                            # 添加到全局（.md 文件、目录束或 .zip 压缩包）
dseam-skillmcp skill add <path> --workspace D:\项目A       # 直接添加到指定工作区
dseam-skillmcp skill scope <name> --global                  # 迁移单个技能到全局
dseam-skillmcp skill scope <name> --workspace D:\项目A      # 迁移单个技能到指定工作区（--copy 复制）
dseam-skillmcp skill migrate <name...|--all> --from <全局|路径> --to <全局|路径> [--copy] [--yes]
dseam-skillmcp skill update [--profile <name>]  # 检查并更新插件（默认 web 配置）
                                                 # 批量迁移（复制/移动）
dseam-skillmcp skill disable <name>       # 停用
dseam-skillmcp skill enable <name>        # 启用
dseam-skillmcp skill delete <name>        # 删除（需确认）
```
...
dseam-skillmcp update [--yes] [--profile <name>]      # 更新整个 dseam-skillmcp
```

MCP 配置写入目标 profile 的 `cordis.patch.yml` 受管块；网关在线时自动热加载。面板块由
`# >>> dseam-skillmcp:mcp:begin` / `# <<< ...end` 标记，请勿手改块内内容。
...
页面和 `dseam-skillmcp skill` 命令的每次操作，最终都是对磁盘上技能文件（`SKILL.md`）的改动，DSH 自带的文件监听器立刻发现变化——所以启用/停用、增删、迁移都热生效，无需重启网关。
